...
<?php
	...
	echo '<a href="user.php"><button id=\'goBack\'><b>Refresh</b></button></a>';
	echo '<br/><br/>';
...
# User info
	...
	foreach ($user as $u) {
		...
		echo '&nbsp;&nbsp;&nbsp;<a href="user.php?action=editLang"><button>Edit</button></a><br/>';
		$id_lang = $u['id_lang'];
	}
	if(isset($_GET['action']) && $_GET['action'] == 'editLang') {
		$aLang = $db->query("SELECT * from pLanguage;");
		echo '<br/><br/>Choose language:<form action="user.php?action=updateLang" method="POST"><select name="id">';
		foreach ($aLang as $aL)
		{	
...
# Add new title to personal list
	echo '<br/><hr><br/><h3>Add position:</h3>
		<form action="user.php?action=addPos" method="POST">
...
# Current watching lists
	...
	foreach ($watching as $w) {
		...
		echo '<td><form action="user.php?action=updateCurrentEp" method="POST">
		...
		echo '<td><form action="user.php?action=updateStatus" method="POST">
		...
		echo '<td><form action="user.php?action=delPos" method="POST">
